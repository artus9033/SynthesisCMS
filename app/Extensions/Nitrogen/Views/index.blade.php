...
function nitrogen_autoplay() {
	if({{ $model->autoplay }}){
		nitrogen_progress.css('width', '0px');
		setTimeout(function(){
			nitrogen_progress.animate({width: nitrogen_progress.parent().width()}, {duration: nitrogen_timer_max, complete: function(){ nitrogen_autoplay(); }});
		}, 1000);
		nitrogen_slider.carousel('next');
	}
}
...
function nitrogen_autoplay_resume(){
	...
	nitrogen_progress.animate({width: nitrogen_progress.parent().width()}, {duration: mDurationLeft, complete: function(){ nitrogen_autoplay(); }});
}
