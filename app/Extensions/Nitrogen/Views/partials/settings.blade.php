		...
		<div class="col s12">
			<p class="col s6 center">
				<input class="filled-in" type="checkbox" id="hasButton" name="hasButton" @if($model->hasButton) checked="checked" @endif>
					<label for="hasButton" class="{{ $synthesiscmsMainColor }}-text">{{ trans("Nitrogen::nitrogen.header_hasButton") }}</label>
				</p>
			</div>
			...
			<div class="col s12">
				<p class="col s6 center">
					<input class="filled-in" type="checkbox" id="assignedToAllPages" name="assignedToAllPages" @if($model->assignedToAllPages) checked="checked" @endif>
						<label for="assignedToAllPages" class="{{ $synthesiscmsMainColor }}-text">{{ trans("Nitrogen::nitrogen.assigned_to_all_pages") }}</label>
					</p>
				</div>
				<style>
				#page-div .caret {
				...
				<ul class="collapsible popout col s12 row" data-collapsible="accordion">
					<li>
						<div class="collapsible-header {{ $synthesiscmsMainColor }}-text" id="pagesCollapsible" style="pointer-events: none;"><i class="material-icons {{ $synthesiscmsMainColor }}-text center">done_all</i>{{ trans("Nitrogen::nitrogen.assigned_pages") }}</div>
						<div class="collapsible-body col s12 card-panel z-depth-3">
							<div class="input-field col s12" id="page-div">
								<select multiple name="assigned_pages[]">
									<option value="" disabled selected>{{ trans("Nitrogen::nitrogen.choose_pages") }}</option>
									@php
									$pages_assigned = $model->assignedPages;
									$pages_assigned_array = explode(";", $pages_assigned);
									@endphp
									@foreach(\App\Models\Content\Page::all() as $page)
										<option class="truncate" value="{{ $page->id }}" @if(in_array($page->id, $pages_assigned_array)) selected @endif>{{ $page->page_title }}</option>
										@endforeach
									</select>
								</div>
							</div>
					...
					<div class="col s12">
						<p class="col s6 center">
							<input class="filled-in" type="checkbox" id="autoplay" name="autoplay" @if($model->autoplay) checked="checked" @endif>
								<label for="autoplay" class="{{ $synthesiscmsMainColor }}-text">{{ trans("Nitrogen::nitrogen.header_autoplay") }}</label>
							</p>
						</div>
						<ul class="collapsible popout col s12 row" data-collapsible="accordion">
							<li>
								<div class="collapsible-header {{ $synthesiscmsMainColor }}-text" id="autoplayCollapsible" style="pointer-events: none;"><i class="material-icons {{ $synthesiscmsMainColor }}-text center">fast_forward</i>{{ trans("Nitrogen::nitrogen.item_autoplay") }}</div>
								<div class="collapsible-body col s12 card-panel z-depth-3">
									<div class="input-field col s12 l6">
										<i class="material-icons prefix {{ $synthesiscmsMainColor }}-text">timelapse</i>
										<input min="500" step="500" type="number" id="interval" name="interval" type="text" value="{{ $model->interval }}">
						...
						<div class="col s12">
							<p class="col s6 center">
								<input class="filled-in" type="checkbox" id="buttons" name="buttons" @if($model->buttons) checked="checked" @endif>
									<label for="buttons" class="{{ $synthesiscmsMainColor }}-text">{{ trans("Nitrogen::nitrogen.header_buttons") }}</label>
								</p>
							</div>
						<script>
