		...
		<div class="card-content @if($synthesiscmsClientIsAnyMobile) no-padding @endif">
			...
			<div class="col s12 row">
				...
				<div class="col s12 hide-on-med-and-up">
						<span class="col s12"><span class="{{ $synthesiscmsMainColor }}-text">{{ trans('synthesiscms/profile.id') }}
								</span>{{ \Auth::user()->id }}</span>
					<div class="col s12 row divider {{ $synthesiscmsMainColor }}-text"></div>
					<span class="col s12"><span class="{{ $synthesiscmsMainColor }}-text">{{ trans('synthesiscms/profile.name') }}
							</span>{{ \Auth::user()->name }}</span>
					<div class="col s12 row divider {{ $synthesiscmsMainColor }}-text"></div>
					<span class="col s12"><span class="{{ $synthesiscmsMainColor }}-text">{{ trans('synthesiscms/profile.email') }}
							</span>{{ \Auth::user()->email }}</span>
					<div class="col s12 row divider {{ $synthesiscmsMainColor }}-text"></div>
					<span class="col s12"><span class="{{ $synthesiscmsMainColor }}-text">{{ trans('synthesiscms/profile.rights') }}
							</span>@php if(\Auth::user()->is_admin){ echo trans('synthesiscms/profile.admin'); }else{ echo trans('synthesiscms/profile.user'); } @endphp</span>
				</div>
				<div class="col s12 l5 row">
					<div class="col s12 row"></div>
					<div class="col s12 l10 offset-l1">
						<a href="{{ url('/profile/password') }}"
						   class="btn btn-large {{ $synthesiscmsMainColor }} {{ $synthesiscmsMainColorClass }} waves-effect waves-light center hoverable col s12 m10 offset-m1"><i
									class="material-icons white-text left">lock_outline</i>{{ trans('synthesiscms/profile.change_password') }}
						</a>
					</div>
					<div class="col s12 row"></div>
					<div class="col s12 l10 offset-l1">
						<a href="{{ route('logout') }}"
						   ...
						   class="btn btn-large col s12 {{ $synthesiscmsMainColor }} {{ $synthesiscmsMainColorClass }} waves-effect waves-light center hoverable col s12 m10 offset-m1"><i
