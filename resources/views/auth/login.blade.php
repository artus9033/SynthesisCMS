			...
			<div class="card-title">
				...
				<form class="form-horizontal col s12" role="form" method="POST" action="{{ url('/login') }}">
					...
					<div class='row'>
						...
						<div class="row"></div>
						<button type='submit' name='btn_login' class='col s12 btn btn-large waves-effect waves-light {{ $synthesiscmsMainColor }} {{ $synthesiscmsMainColorClass }} hoverable'>{{ trans('synthesiscms/auth.login_btn')}}</button>
					</div>
				</form>
			</div>
			<div class="card-action col s12 row center">
				<a class="col s4 offset-s2 center {{ $synthesiscmsMainColor }}-text darken-1" href="{{ url('/register') }}">{{ trans('synthesiscms/auth.register')}}</a>
				<a class="col s4 center {{ $synthesiscmsMainColor }}-text darken-1" href="{{ url('/password/reset') }}">{{ trans('synthesiscms/auth.reset')}}</a>
			</div>
		</div>
	@endsection
