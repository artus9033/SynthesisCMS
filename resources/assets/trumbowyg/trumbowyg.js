...
(function(navigator, window, document, $) {
  ...
  var Trumbowyg = function(editorElem, options) {
    ...
    t.hideButtonTexts =
      $.trumbowyg.hideButtonTexts != null
        ? $.trumbowyg.hideButtonTexts
        : options.hideButtonTexts;

    // SVG path
    var svgPathOption =
      $.trumbowyg.svgPath != null ? $.trumbowyg.svgPath : options.svgPath;
    t.hasSvg = svgPathOption !== false;
    t.svgPath = !!t.doc.querySelector("base")
      ? window.location.href.split("#")[0]
      : "";
    if (
      $("#" + trumbowygIconsId, t.doc).length === 0 &&
      svgPathOption !== false
    ) {
        ...
        try {
          ...
          if (!e.hasOwnProperty("stack")) {
            ...
            for (var i in stackLines) {
              ...
              svgPathOption = stackLines[Number(i)]
                .match(/((http[s]?:\/\/.+\/)([^\/]+\.js))(\?.*)?:/)[1]
                .split("/");
              svgPathOption.pop();
    ...
    t.disabled =
      t.o.disabled ||
      (editorElem.nodeName === "TEXTAREA" && editorElem.disabled);
  ...
  Trumbowyg.prototype = {
    ...
    buildEditor: function() {
        ...
        .on("focus blur", function(e) {
          ...
          if (e.type === "blur") {
            $("." + prefix + "active-button", t.$btnPane).removeClass(
              prefix + "active-button " + prefix + "active"
            );
          }
          if (t.o.autogrowOnEnter) {
      ...
      t.$box.on("keydown", function(e) {
        if (
          e.which === 27 &&
          $("." + prefix + "modal-box", t.$box).length === 1
        ) {
          t.closeModal();
          return false;
        }
      ...
      $.each(t.o.btns, function(i, btnGrps) {
        ...
        var $btnGroup = $("<div/>", {
          class:
            prefix +
            "button-group " +
            (btnGrps.indexOf("fullscreen") >= 0 ? prefix + "right" : "")
        });
      ...
      var t = this,
        ...
        $btn = $("<button/>", {
          ...
          html:
            t.hasSvg && hasIcon
              ? '<svg><use xlink:href="' +
                ...
                "#" +
                prefix +
                (btn.ico || btnName).replace(/([A-Z]+)/g, "-$1").toLowerCase() +
                '"/></svg>'
              : t.hideButtonTexts
              ? ""
          ...
          title:
            ...
            (btn.key ? " (Ctrl + " + btn.key + ")" : ""),
          tabindex: -1,
          mousedown: function() {
            if (
              !isDropdown ||
              $("." + btnName + "-" + prefix + "dropdown", t.$box).is(":hidden")
            ) {
            ...
            t.execCmd(
              (isDropdown ? "dropdown" : false) || btn.fn || btnName,
              btn.param || btnName,
              btn.forceCss || false
            );
      ...
      if (show) {
        ...
        $dropdown
          .css({
            position: "absolute",
            top:
              $btn.offset().top - t.$btnPane.offset().top + $btn.outerHeight(),
            left:
              t.o.fixedFullWidth && t.isFixed
                ? o + "px"
                : o - t.$btnPane.offset().left + "px"
          })
          .show();
    ...
    syncTextarea: function() {
      ...
      t.$ta.val(
        t.$ed.text().trim().length > 0 ||
          t.$ed.find("hr,img,embed,iframe,input").length > 0
          ? t.$ed.html()
          : ""
      );
    },
        ...
        .on(CONFIRM_EVENT, function() {
          ...
          $.each(fields, function(fieldName, field) {
            ...
            if (field.required && values[fieldName] === "") {
              ...
              field.pattern &&
              !field.pattern.test(values[fieldName])
            ) {
              valid = false;
      ...
      $.each(tags, function(i, tag) {
        ...
        if ($btn.length > 0) {
          ...
          try {
            ...
            $("." + prefix + dropdownBtnName + "-button", t.$box).addClass(
              activeClasses
            );
          } catch (e) {}
        }
