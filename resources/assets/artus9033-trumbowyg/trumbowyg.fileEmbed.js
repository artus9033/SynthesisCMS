...
function trumbowygFilePickerCallback(url, fsize) {
	if (url.length) {
		...
		var body = $("<body></body>");
		var card = $(
			"<div class='synthesis-dynamic-url-file-wrapper col s12 l8 offset-l2 card'></div>"
		);
		var cardContent = $("<div class='card-content'></div>");
		var cardDescription = $("<p>" + fileName + " (" + fsize + ")" + "</p>");
		var cardActionDiv = $("<div class='card-action'></div>");
		var cardActionItem = $(
			"<a target='_blank' class='truncate'><i class='material-icons'>file_download</i>" +
				prompt("Download button text", "download") +
				"</a>"
		);
		var script = $(
			"<script>$(document).ready(function(){SynthesisCmsJsUtils.triggerSynthesisDynamicUrlsRescanOnElement('.synthesis-dynamic-url-file-wrapper');});"
		);
		cardActionItem = SynthesisCmsJsUtils.addDynamicSynthesisUrlClientSideProcessableAttributesToElement(
			cardActionItem,
			url,
			"href"
		);
		...
		var materializecss = $(
			'<link type="text/css" rel="stylesheet" href="' +
				mAssetBaseUrl +
				'css/materialize.css" media="screen,projection"/>'
		);
		var materialicons = $(
			'<link type="text/css" rel="stylesheet" href="' +
				mAssetBaseUrl +
		...
		var synthesiscmsjsutils = $(
			'<script type="text/javascript" src="' +
				mAssetBaseUrl +
				'js/synthesiscms-js-utils.js"></script>'
		);
		var metaSiteRoot = $(
			'<meta name="synthesiscms-public-root" content="' +
				SynthesisCmsJsUtils.getSiteRootUrl() +
				'">'
		);
		var metaAssetRoot = $(
			'<meta name="synthesiscms-asset-root" content="' + mAssetBaseUrl + '">'
		);
		...
		html.append(body);
		var iframe = $(
			'<iframe class="col s12 l8 offset-l2" frameborder="0" scrolling="yes" marginheight="0" marginwidth="0"></iframe>'
		);
...
SynthesisCmsJsUtils.includeFilePickerDynamically(
	"trumbowyg_embed_file_picker",
	"trumbowygFilePickerCallback",
	["*"]
);
		...
		plugins: {
			fileEmbed: {
				init: function(trumbowyg) {
					trumbowyg.o.plugins.fileEmbed = $.extend(
						true,
						{},
						{},
						trumbowyg.o.plugins.fileEmbed || {}
					);
